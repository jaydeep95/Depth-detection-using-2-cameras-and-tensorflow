...
def classification(val1,val2,cam_property,frame_left,frame_right):
  ...
  for i in val1:# iterating through first set of values camera 1
    ...
    if number_of_match >= 1:
      ...
      '''else: # if matching only one row of frame right
        temp_row2 = lst_of_mat_row[0]
        val = ratio_find(val1,val2,row1-1,temp_row2)
        print ("ratio value is %f"%val)
        if val>85 and val <115:
          hist_left = hist_check(frame_left,val1,row1-1,cam_property)
          hist_right = hist_check(frame_right,val2,temp_row2,cam_property)
...
left = 1
right = 0
cap1 = cv2.VideoCapture('office_nw_right.avi')#(left)#('cutnw_left.avi')
cap2 = cv2.VideoCapture('office_nw_left.avi')#(right)#('cutnw_right.avi')
